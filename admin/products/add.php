...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("INSERT INTO products (category_id, name, slug, small_description, description, original_price, selling_price, qty, status, image, image1, image2, image3, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
        $stmt->bind_param("isssssdiissss", $category_id, $name, $slug, $small_description, $description, $original_price, $selling_price, $qty, $status, $image, $image1, $image2, $image3);
        if ($stmt->execute()) {
        ...
        <div class="container mt-5">
            ...
            <form method="post" enctype="multipart/form-data" class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="original_price" class="form-control" min="100000" step="100000" value="<?= isset($original_price) ? htmlspecialchars($original_price) : '10000000' ?>">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Giá bán</label>
                    <input type="number" name="selling_price" class="form-control" min="100000" step="100000" required value="<?= isset($selling_price) ? htmlspecialchars($selling_price) : '10000000' ?>">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Số lượng</label>
                    <input type="number" name="qty" class="form-control" min="0" required value="<?= isset($qty) ? htmlspecialchars($qty) : '' ?>">
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-12">
                    ...
                    <textarea name="small_description" class="form-control" rows="2"><?= isset($small_description) ? htmlspecialchars($small_description) : '' ?></textarea>
                </div>
                <div class="col-md-6 d-flex align-items-end">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="status" id="status" <?= (isset($status) && $status == 1) ? 'checked' : '' ?>>
