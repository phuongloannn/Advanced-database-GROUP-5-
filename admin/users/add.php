...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($role_as == 1) {
        $errors[] = "Không thể tạo tài khoản admin";
        $role_as = 0; // Đặt lại thành tài khoản khách
    }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO users (name, email, password, role_as, creat_at) VALUES (?, ?, ?, ?, NOW())");
        $stmt->bind_param("sssi", $name, $email, $hashed_password, $role_as);
        if ($stmt->execute()) {
            ...
            <form method="post" class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="role_as" class="form-select">
                        <option value="0" <?= (isset($role_as) && $role_as == 0) ? 'selected' : '' ?>>Khách</option>
                        <option value="2" <?= (isset($role_as) && $role_as == 2) ? 'selected' : '' ?>>Nhân viên</option>
                    </select>
                </div>
                <div class="col-12">
